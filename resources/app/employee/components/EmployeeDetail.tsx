...
import { IconEdit, IconArrowLeft, IconUser, IconBriefcase, IconSchool, IconCertificate, IconLicense } from '@tabler/icons-react';
        ...
        <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Badge variant={employee.status_kepegawaian === 'active' ? 'success' : 'secondary'}>
      ...
      <Tabs defaultValue="personal" className="w-full">
          ...
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="font-medium">{formatDate(employee.tanggal_lahir)}</p>
              </div>
              {employee.alamat && (
          ...
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="font-medium">{formatDate(employee.tanggal_masuk)}</p>
              </div>
              {employee.tanggal_keluar && (
                <div>
                  <p className="text-sm text-gray-600">Tanggal Keluar</p>
                  <p className="font-medium">{formatDate(employee.tanggal_keluar)}</p>
                </div>
              )}
              <div>
              ...
              {employee.manager && (
                <div className="sm:col-span-2">
                  ...
                  >
                    {employee.manager.nama_lengkap} ({employee.manager.nomor_karyawan})
                  </Link>
                </div>
              )}
            </div>
        ...
        <TabsContent value="education" className="mt-4">
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            ...
            {employee.education_history && employee.education_history.length > 0 ? (
                ...
                {employee.education_history.map((edu) => (
                  <div key={edu.id} className="border-l-4 border-blue-500 pl-4">
                    ...
                    <div className="mt-2 flex gap-4 text-sm text-gray-600">
                      {edu.graduation_year && <span>Lulus: {edu.graduation_year}</span>}
              ...
              <p className="text-center text-gray-500">Belum ada riwayat pendidikan</p>
            )}
          </div>
        </TabsContent>
            ...
            {employee.certifications && employee.certifications.length > 0 ? (
              <div className="space-y-4">
                {employee.certifications.map((cert) => (
                  <div key={cert.id} className="border-l-4 border-green-500 pl-4">
                    <div className="flex items-start justify-between">
                      <div>
                    ...
                    <div className="mt-2 flex gap-4 text-sm text-gray-600">
                      ...
                      {cert.expiry_date && (
                        <span>Berlaku sampai: {formatDate(cert.expiry_date)}</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <TabsContent value="licenses" className="mt-4">
          <div className="rounded-lg border bg-white p-6 shadow-sm">
            ...
            {employee.professional_licenses && employee.professional_licenses.length > 0 ? (
              <div className="space-y-4">
                {employee.professional_licenses.map((license) => (
                  <div key={license.id} className="border-l-4 border-purple-500 pl-4">
                    <div className="flex items-start justify-between">
                      <div>
                    ...
                    <div className="mt-2 flex gap-4 text-sm text-gray-600">
                      ...
                      <span>Berlaku sampai: {formatDate(license.expiry_date)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-center text-gray-500">Belum ada lisensi profesional</p>
            )}
          </div>
        </TabsContent>
      </Tabs>
